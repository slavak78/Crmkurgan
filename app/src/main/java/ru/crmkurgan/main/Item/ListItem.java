...
public class ListItem extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, final int position) {
            ItemRowHolder rt = (ItemRowHolder) holder;
            final PropertyModels singleItem = dataList.get(position);
            Spanned html = Html.fromHtml(singleItem.getName());
            rt.text1.setText(html);
            rt.price1.setText(singleItem.getPrice());
            rt.address1.setText(Html.fromHtml(singleItem.getAddress()));
            rt.ratingView.setRating(Float.parseFloat(singleItem.getRateAvg()));
            Picasso.get()
            ...
            if (databaseHelper.getFavouriteById(singleItem.getPropid(),singleItem.getTabl())) {
                rt.favourite.setColorFilter(mContext.getResources().getColor(R.color.red));
            } else {
                rt.favourite.setColorFilter(mContext.getResources().getColor(R.color.gray));
            }

            rt.favourite.setOnClickListener(view -> {
            ...
            rt.rootlayout.setOnClickListener(v -> {
